...
export default function GroupPage() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            const betsResponse = await fetch(`/api/groups/${groupId}/bets`);
            const { result: betsResult } = await betsResponse.json();
            setBets(betsResult);
        }
        if (session) {
            fetchData();
        ...
        <Page>
            <Section>
                <LoadingSection loading={loading && !group}>
                    ...
                    <h3>Active Bets</h3>
                    {(bets && bets.length && bets.map(bet => (
                        <div key={bet.id}>
                            <div className="bet" v-bind:class="{ 'even': index%2 !== 0 }">
                                ...
                                <div>
                                    ...
                                    <p className="status" v-bind:class="{'no-bet': bet.stake === 0}">
                    ...
                    <Link href="{ name: 'Add Bet', query: { groupId: $route.params.id } }">
                        <button>+ bet</button>
                    </Link>
                    <h3>Leaderboard</h3>
